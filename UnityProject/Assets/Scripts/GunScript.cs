...
public class GunScript:MonoBehaviour{
    ...
    public void Start() {
        ...
        if(magazineType == MagazineType.MAGAZINE || magazineType == MagazineType.INTERNAL) {
            ...
            if(hasSlideLock && Random.Range(0, 2) == 0) {
    ...
    public bool ApplyPressureToTrigger() {
        ...
        if(action_type == ActionType.DOUBLE && trigger_pressed < 1f && thumb_on_hammer == Thumb.OFF_HAMMER) {
            CockHammer();
            CockHammer();
        }
    ...
    public bool ToggleSafety() {
        ...
        if(safety == Safety.OFF) {
            if(slide_amount == 0f && hammer_cocked == 1f) {
                safety = Safety.ON;
    ...
    public void CockHammer() {
        ...
        hammer_cocked = Mathf.Min(1f, hammer_cocked + Time.deltaTime * 10f);
        if(hammer_cocked == 1f && old_hammer_cocked != 1f) {
            if(thumb_on_hammer == Thumb.ON_HAMMER) {
                PlaySoundFromGroup(sound_hammer_cock, kGunMechanicVolume);
            }
            ++active_cylinder;
            cylinder_rotation = active_cylinder * 360f / cylinder_capacity;
        }
        if(hammer_cocked < 1f) {
            cylinder_rotation = (active_cylinder + hammer_cocked) * 360f / cylinder_capacity;
            target_cylinder_offset = (int)0f;
        }
    }
    ...
    public void ReleaseHammer() {
        ...
        if((pressure_on_trigger != PressureState.NONE && safety_off == 1f) || hammer_cocked != 1f) {
    ...
    public bool IsHammerCocked() {
        return hammer_cocked == 1f;
    }
    
    public bool ShouldPullBackHammer() {
        return hammer_cocked != 1f && has_hammer && action_type == ActionType.SINGLE;
    }
    ...
    public void Update() {
        if(magazineType != MagazineType.CYLINDER) {
            ...
            if(mag_stage == MagStage.INSERTING) {
                ...
                if(mag_seated >= 1f) {
                    ...
                    recoil_transfer_y += Random.Range(-40f, 40f);
                    recoil_transfer_x += Random.Range(50f, 300f);
                    rotation_transfer_x += Random.Range(-0.4f, 0.4f);
                    rotation_transfer_y += Random.Range(0f, 1f);
                }
            }
        ...
        if(has_safety) {
            if(safety == Safety.OFF) {
                safety_off = Mathf.Min(1f, safety_off + Time.deltaTime * 10f);
            } else if(safety == Safety.ON) {
                safety_off = Mathf.Max(0f, safety_off - Time.deltaTime * 10f);
            }
        }
        
        if(has_auto_mod) {
            if(auto_mod_stage == AutoModStage.ENABLED) {
                auto_mod_amount = Mathf.Min(1f, auto_mod_amount + Time.deltaTime * 10f);
            } else if(auto_mod_stage == AutoModStage.DISABLED) {
                auto_mod_amount = Mathf.Max(0f, auto_mod_amount - Time.deltaTime * 10f);
            }
        }
        
        if(thumb_on_hammer == Thumb.SLOW_LOWERING) {
            hammer_cocked -= Time.deltaTime * 10f;
            if(hammer_cocked <= 0f) {
                hammer_cocked = 0f;
                thumb_on_hammer = Thumb.OFF_HAMMER;
        ...
        if(has_slide) {
            if(slide_stage == SlideStage.NOTHING) {
                ...
                slide_amount = Mathf.Max(0f, slide_amount - Time.deltaTime * kSlideLockSpeed);

                if(!slide_lock && slide_amount == 0f && old_slide_amount != 0f) {
                ...
                if(slide_amount == 0f && round_in_chamber_state == RoundState.LOADING) {
        ...
        if(gun_type == GunType.REVOLVER) {
            ...
            if(yolk_stage == YolkStage.CLOSING) {
                yolk_open -= Time.deltaTime * 5f;
                if(yolk_open <= 0f) {
                    yolk_open = 0f;
                    yolk_stage = YolkStage.CLOSED;
                    PlaySoundFromGroup(sound_cylinder_close, kGunMechanicVolume * 2f);
                    target_cylinder_offset = 0;
                }
            }
            ...
            if(yolk_stage == YolkStage.OPENING) {
                ...
                if(yolk_open >= 1f) {
                    ...
                    PlaySoundFromGroup(sound_cylinder_open, kGunMechanicVolume * 2f);
                }
            }

            if(extractor_rod_stage == ExtractorRodStage.CLOSING) {
                extractor_rod_amount -= Time.deltaTime * 10f;
                if(extractor_rod_amount <= 0f) {
                    extractor_rod_amount = 0f;
            ...
            if(extractor_rod_stage == ExtractorRodStage.OPENING) {
                ...
                if(extractor_rod_amount >= 1f) {
                    if(!extracted) {
                        for(int i=0; i<cylinder_capacity; ++i) {
                            if(cylinders[i].game_object) {
                                if(Random.Range(0f, 3f) > cylinders[i].seated) {
                    ...
                    for(int i=0; i<cylinder_capacity; ++i) {
                        if(cylinders[i].game_object && cylinders[i].falling) {
                            cylinders[i].seated -= Time.deltaTime * 5f;
                            if(cylinders[i].seated <= 0f) {

...
public class AimScript:MonoBehaviour{
    ...
    public bool IsAiming() {
        bool Aiming = (VRInputController.instance.GetAimDir(primaryHand).y > -0.5f);
        if (Aiming) {
    ...
    public void HandleGunControls(bool insert_mag_with_number_key) {
        ...
        if (!VRInputBridge.instance.MagOut && character_input.GetButton("Pull Back Slide", secondaryHand)) {
    		if(character_input.GetButton("Slide Lock", primaryHand)){
    			gun_script.Request(GunSystemRequests.INPUT_PULL_SLIDE_PRESS_CHECK);
    		} else if (character_input.GetButtonUp("Slide Lock", primaryHand)){
...
>>>>>>> flatgame-repo/master
    	...
    	*/
        ...
        x_recoil_spring.vel += recoil_transfer.x * (VRInputController.instance.isFrontGrabbing ? 0.1f : 1f);
        y_recoil_spring.vel += recoil_transfer.y * (VRInputController.instance.isFrontGrabbing ? 0.1f : 1f);
        rotation_x += rotation_transfer.x * (VRInputController.instance.isFrontGrabbing ? 0.1f : 1f);
        rotation_y += rotation_transfer.y * (VRInputController.instance.isFrontGrabbing ? 0.1f : 1f);
    ...
    public void UpdateCameraAndPlayerTransformation() {
        ...
        Vector3 AdaptedForward = character_controller.transform.InverseTransformDirection(main_camera.transform.forward) * 0.15f;

        character_controller.center = new Vector3(main_camera.transform.localPosition.x - (AdaptedForward.x), 0, main_camera.transform.localPosition.z - (AdaptedForward.z)) * main_camera.transform.root.localScale.x;// - main_camera.transform.parent.rotation * new Vector3(startOffset.x, 0, startOffset.z);
    ...
    public void UpdateMagazineTransformation() {
       ...
       if(mag_stage == HandMagStage.HOLD || mag_stage == HandMagStage.HOLD_TO_INSERT){
       		...
       		Vector3 hold_pos = VRInputController.instance.GetAimPos(secondaryHand,true);
    ...
    private void DrawHelpLine(string text, bool bold = false, bool clear = false) {
        /*
        ...
        if (clear) {
            HelpCanvasManager.helpText.text = "";
        }
        else {
            if (!bold) {
                //HelpCanvasManager.helpText.text += "<color=black>" + text + "\n</color>";
            }
            else {
                HelpCanvasManager.helpText.text += "<color=white>" + text + "\n</color>";
            }
        }
    }
        ...
        if (main_client_control && Event.current.type == EventType.Repaint) {
            ...
            if (PlayerPrefs.GetInt("show_help",0) == 0) {
                ...
                /*if (held_flashlight != null) {
                    ...
                    if (empty_slot != -1) {
                        DrawHelpLine($"Put flashlight in inventory: trigger [ {empty_slot} ]");
                    }
                    ...
                    if (flashlight_slot != -1) {
                        DrawHelpLine($"Equip flashlight: trigger [ {flashlight_slot} ]", true);
                    }
                }*/
                if (gun_instance != null) {
                    ...
                    DrawHelpLine("Holster your weapon", ShouldHolsterGun());
                }
                else {
                    DrawHelpLine("Draw your weapon", ShouldDrawWeapon());
                }
                if (gun_instance != null) {
                    if (gun_script.HasSlide()) {
                        if (gun_script.Query(GunSystemQueries.IS_WAITING_FOR_SLIDE_PUSH)) {
                            DrawHelpLine("Push forward slide", gun_script.ShouldPushSlideForward());
                        }
                        else {
